...
- [x] 이전에 만든 `render` 함수를 개선하여 `diff` 알고리즘을 추가하세요.

- [x] 상태 변경 시 새로운 Virtual DOM을 생성하고, 이전 Virtual DOM과 비교하여 변경된 부분만 업데이트하세요.
...
- [ ] JSX에서 이벤트를 처리하는 방법을 학습하고, 우리 라이브러리에 적용하세요.

- [x] onClick, onChange 등의 이벤트 핸들러를 Virtual DOM에서 처리하도록 구현하세요.

- [ ] Synthetic Event 시스템을 간단히 구현하여 이벤트 객체를 관리하세요.

#### 📚 DAY5

**TODO LIST**

- [x] useEffect 훅을 구현하여 컴포넌트가 마운트될 때와 업데이트될 때 특정 작업을 수행하세요.

- [x] 의존성 배열을 활용하여 필요할 때만 이펙트가 실행되도록 최적화하세요.

- [x] 타이머나 이벤트 리스너를 등록하고, 컴포넌트 언마운트 시 클린업하세요.
